/**
 * Ripped straight from CVA Demonstrator...
 */
AFRAME.registerComponent("vr-hand-grab", {
	...
	tick: function (time, deltaTime) {
		...
		const gamepad = (this.el.components["tracked-controls"]?.controller?.gamepad)
			|| (this.el.components["oculus-touch-controls"]?.controller?.gamepad);
		const aPressed = gamepad?.buttons[4]?.pressed; // A button
		const bPressed = gamepad?.buttons[5]?.pressed; // B button
		...
		if (window.vrMode === "menu") {
			...
			hand.setAttribute("super-hands", {
				...
				grabStartButtons: [], // disable grabbing in menu
				stretchStartButtons: [],
				dragDropStartButtons: []
			});

			if (this.heldEl) this.drop(); // drop any held object
			this.wasGrabPressed = false;
			return;
		}
		...
		// Move player up/down with A/B
		const player = this.el.sceneEl.querySelector("#player");
		if (player) {
			const step = 0.001 * deltaTime; // framerate-independent speed
			if (aPressed) player.object3D.position.y += step;
			if (bPressed) player.object3D.position.y -= step;
		}
